...
X, y = experiment_data.load_data_svm(load_mode='std_overlap')
...
for counter, ii in enumerate(cv):
    for C in C_range:
        classifier = svm.SVC(kernel='linear',C=C, random_state=random_state)
...
df.to_csv('../data/svm_training_LOSO_linear_kernel_std_overlap.csv')
